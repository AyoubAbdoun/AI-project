...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState: GameState):
        ...
        def minimax(state, agentIndex, depth):
            # Terminal test: reached max depth or game over
            ...
            # If there are no legal actions, evaluate the state
            if not actions:
                return self.evaluationFunction(state)

            # Compute next agent and depth (depth counts full Pacman moves)
            ...
            if agentIndex == 0:
                ...
                for action in actions:
                    ...
                    value = max(value, minimax(successor, nextAgent, nextDepth))
                return value
        ...
        # Root call: choose the best action for Pacman (agent 0)
        bestValue = -float("inf")
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState: GameState):
        ...
        # Root: Pacman chooses best action
        bestValue = -float("inf")
        bestAction = Directions.STOP
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState: GameState):
        ...
        def expectimax(state, agentIndex, depth):
            ...
            # Ghosts (chance nodes, uniform over actions)
            total = 0.0
            for action in actions:
                successor = state.generateSuccessor(agentIndex, action)
        ...
        # Root: choose action for Pacman (agent 0)
        bestValue = -float("inf")
        bestAction = Directions.STOP
